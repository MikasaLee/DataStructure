...
#define MAXSTRLEN 25
typedef unsigned char SString[MANSTRLEN+1];	//下标为0用来存放串的长度
...
int StrCompare(SString str1,SString str2);	//比较str1和str2的大小，大于返回1等于返回0小于返回-1
...
Status SubString(SString sub,SString S,int pos,int len);	//用sub返回串S的[pos,len)的子串
...
Status StrInsert(SString S,int pos,SString T);	//在串S的第pos个字符之前插入串S
