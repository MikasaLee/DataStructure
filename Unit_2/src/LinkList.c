...
int LocateElem_L(LNode L,ElemType e,Bool (*compare)(ElemType a,ElemType b)){
	int i =1;
	LinkList p = (&L);
	p = p ->next;
...
Status PriorElem_L(LNode L,ElemType cur_e,ElemType *pre_e){
	LinkList p,q;
	if(!L.next){
		printf("-----该表为空表，操作失败-----\n");
		return ERROR;
...
Status NextElem_L(LNode L,ElemType cur_e,ElemType *next_e){
	LinkList p,q;
	if(!L.next){
		printf("-----该表为空表，操作失败-----\n");
		return ERROR;
